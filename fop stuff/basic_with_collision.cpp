...
int main()
{
	...
	string zombieMess = "";
	string cheatMess = "";
	bool zombiesCanMove = true;
	bool zombiesWereKilled = false;
	bool cheated = false;
	...
	endProgram(stats, username);						//display final message
	return 0;
}
...
{ //initialise grid and place spot in middle
	...
	void setSpotInitialCoordinates(Item& spot, char g[][SIZEX]);
	...
	void setpills(vector<Item>& pills, char g[][SIZEX], vector<Item>& holes, vector<Item>& zombies, Item& spot);
	//void setzombiecoordinates(vector<Item>& zombies);
	...
	//setSpotInitialCoordinates(spot, grid);
	setpills(pills, grid, holes, zombies, spot);
...
void setpills(vector<Item>& pills, char g[][SIZEX], vector<Item>& holes, vector<Item>& zombies, Item& spot) {
	...
	for (size_t i(0); i < pills.size(); ++i) {
		...
		while ((grid2[pills.at(i).y][pills.at(i).x] == 'Z') || (g[pills.at(i).x][pills.at(i).y] == SPOT)) {
			pills.at(i).x = Random(SIZEX - 2);
			pills.at(i).y = Random(SIZEY - 2);
			pills.at(i).symbol = POWERPILL;
		}
