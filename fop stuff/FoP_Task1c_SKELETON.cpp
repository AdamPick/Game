...
int main()
{
	...
	void initialiseGame(char g[][SIZEX], char m[][SIZEX], Item& spot, vector<Item>& holes, Item& powerpill);
	void paintGame(const char g[][SIZEX], string mess, Stats stats);
	bool wantsToQuit(const int key);
	bool isArrowKey(const int k);
	int  getKeyPress();
	void updateGameData(const char g[][SIZEX], Item& spot, const int key, string& mess, Stats& stats, char m[][SIZEX]);
	void updateGrid(char g[][SIZEX], const char m[][SIZEX], const Item spot, vector<Item>& holes, Item& powerpill);
	...
	initialiseGame(grid, maze, spot, holes, powerpill);	//initialise grid (incl. walls and spot)
	...
	do {
		...
		if (isArrowKey(key))
		{
			updateGameData(grid, spot, key, message, stats, maze);		//move spot in that direction
			updateGrid(grid, maze, spot, holes, powerpill);					//update grid information
		}
		else
...
void initialiseGame(char grid[][SIZEX], char maze[][SIZEX], Item& spot, vector<Item>& holes, Item& powerpill)
{ //initialise grid and place spot in middle
	void setInitialMazeStructure(char maze[][SIZEX]);
	void setSpotInitialCoordinates(Item& spot);
	void setholes(vector<Item>& holes, Item spot);
	void updateGrid(char g[][SIZEX], const char m[][SIZEX], Item b, vector<Item>& holes, Item& powerpill);
	...
	setholes(holes, spot); //sets coordinates of holes
	...
	for (int i = 0; i < 8; i++) {
		...
		do {
			y = Random(SIZEY - 2);
			x = Random(SIZEX - 2);
		} while (grid[y][x] == TUNNEL);
		maze[y][x] = powerpill.symbol;



	}
	updateGrid(grid, maze, spot, holes, powerpill);		//prepare grid
...
void setholes(vector<Item>& holes, Item spot) { //sets the coordinates of 12 holes
...
void updateGrid(char grid[][SIZEX], const char maze[][SIZEX], const Item spot, vector<Item>& holes, Item& powerpill)
