...
int main()
{
	...
	void initialiseGame(char g[][SIZEX], char m[][SIZEX], Item& spot, vector<Item>& holes, vector<Item>& pills, vector<Item>& zombies, gameData level);
	...
	int  getKeyPress();
	void updateGameData(const char g[][SIZEX], Item& spot, const int key, string& mess, string& spotMess, string& zombieMess, Stats& stats, char m[][SIZEX], vector<Item>& pills, vector<Item>& zombies, int& movementcount, gameData level);
	...
	bool selectLevelScreen(int& levelID);
	...
	Stats stats = { 5, 10, MAXZOMBIES, -1, 0, 0 };
	...
	start = false;
	while (!start) {
		start = selectLevelScreen(levelID);
	}
	gameData level;
	level.levelNumber = levelID;
	vector<Item> holes(5);      //Creates vector where the different holes and their coordinates will be stored
	vector<Item> pills(5);		//Creates vector where the different pills and their coordinates will be stored
	do {
		holes.clear();
		pills.clear();
		zombies.clear();
		...
		case 1:
			...
			level.lives = 8;
			level.powerPills = 8;
			level.numberOfProtectedMoves = 10;
			break;
		case 2:
			level.holes = 5;
			level.lives = 5;
			level.powerPills = 5;
			level.numberOfProtectedMoves = 8;
			break;
		case 3:
			level.holes = 2;
			level.lives = 3;
			level.powerPills = 2;
			level.numberOfProtectedMoves = 5;
			break;
		}

		for (int i = 0; i < level.holes; i++) {
			holes.push_back({ 0,0,'0' });
		}
		for (int i = 0; i < level.powerPills; i++) {
			pills.push_back({ 0,0,'*' });
		}
		for (int i = 0; i < 4; i++) {
			zombies.push_back({ 0,0,'Z' });
		}


		stats.powerpillsRemaining = level.powerPills;
		stats.lives = level.lives;
		initialiseGame(grid, maze, spot, holes, pills, zombies, level);	//initialise grid (incl. walls and spot)
		...
		do {
			...
			{
				stats.movesTaken += 1;
				updateGameData(grid, spot, key, message, spotMess, zombieMess, stats, maze, pills, zombies, movementcount, level);		//move spot in that direction
				...
				{
					message = "LOADED GAME!";
					ifstream fin("leveldata/" + username + "game.txt");
					if (fin.fail()) {
						message = "NO SAVE GAME FILE!";
					}
					else {
						...
						for (int row(0); row < SIZEY; ++row)
							for (int col(0); col < SIZEX; ++col)
								...
								case ' ':
									...
									break;
								case '*':
									pills.push_back({ col, row, POWERPILL });
									break;
								case 'Z':
									zombies.push_back({ col, row, ZOMBIE });
									break;
								case '0':
									holes.push_back({ col, row, HOLE });
						...
						stats.powerpillsRemaining = pills.size();
						stats.zombiesRemaining = zombies.size();
						updateGrid(grid, maze, spot, holes, pills, zombies);
					}
				...
				case 'S':
				{
					message = "SAVED GAME!";
					ofstream fout;
					fout.open("leveldata/" + username + "game.txt", ios::out);
					for (int i = 0; i < SIZEY; i++) {
						for (int j = 0; j < SIZEX; j++) {
							fout.put(grid[i][j]);
						}
		...
		if (!cheated) {
			stats.score = stats.score + (stats.lives* 20) - stats.movesTaken;
		}
		if (level.levelNumber != 3) {
			level.levelNumber += 1;
		}
		else wantsToQuit = true;
	} while (!wantsToQuit);
...
bool entryScreen(string& username) { //entry screen of game
	...
	showMessage(clDarkGrey, clYellow, 42, 9, "TO LOAD GAME PRESS 'L'");
...
bool displayInfo(const char g[][SIZEX]) {
	...
	system("color 86");
	paintGrid(g);
	showMessage(clDarkGrey, clYellow, 26, 2, "WHEN SPOT EATS A POWERPILL, HE GAINS ONE LIFE");
	showMessage(clDarkGrey, clYellow, 26, 5, "WHEN SPOT FALLS INTO A HOLE, HE LOSES ONE LIFE");
	showMessage(clDarkGrey, clYellow, 26, 7, "WHEN SPOT HITS A ZOMBIE HE LOSES ONE LIFE");
	showMessage(clDarkGrey, clYellow, 26, 8, "AND THE ZOMBIE RETURNS TO ITS CORNER");
	showMessage(clDarkGrey, clYellow, 26, 10, "WHEN SPOT EATS A POWERPILL, HE GAINS ONE LIFE");
	showMessage(clDarkGrey, clYellow, 26, 12, "SPOT AND ZOMBIES CANNOT LEAVE THE BOUNDARIES");
	showMessage(clDarkGrey, clYellow, 26, 14, "WHEN ZOMBIES BUMP INTO EACH OTHER,");
	showMessage(clDarkGrey, clYellow, 26, 15, "THEY RETURN TO THEIR CORNERS");
	showMessage(clDarkGrey, clYellow, 26, 17, "WHEN A ZOMBIE FALLS DOWN A HOLE, IT DIES AND DOES");
	showMessage(clDarkGrey, clYellow, 26, 18, "NOT RETURN");
	key = getKeyPress();
	if (toupper(key) == 'I') {
		system("cls");
		return false;
...
bool menuScreen(Stats stats, bool& wantsToQuit) {
	...
	do {
		...
		case 'S':
			system("cls");
			showMessage(clDarkGrey, clYellow, 25, 10, "CURRENT SCORE: " + toString(stats.score));
...
bool selectLevelScreen(int& levelID) {
	...
	showMessage(clDarkGrey, clYellow, 25, 10, "SELECT LEVEL");
	showMessage(clDarkGrey, clYellow, 25, 14, "'1' - EASY");
	showMessage(clDarkGrey, clYellow, 25, 15, "'2' - INTERMEDIATE");
	showMessage(clDarkGrey, clYellow, 25, 16, "'3' - HARD");
	showMessage(clDarkGrey, clYellow, 50, 20, "");
...
void initialiseGame(char grid[][SIZEX], char maze[][SIZEX], Item& spot, vector<Item>& holes, vector<Item>& pills, vector<Item>& zombies, gameData level)
{ //initialise grid and place spot in middle
	void setInitialMazeStructure(char maze[][SIZEX], char grid[][SIZEX], gameData level);
	...
	setInitialMazeStructure(maze, grid, level);		//initialise maze
...
void setInitialMazeStructure(char maze[][SIZEX], char grid[][SIZEX], gameData level)
{
	string toString(int);
	char initialMaze[SIZEY][SIZEX]; 
	ifstream fin("levels/level" + toString(level.levelNumber)+".txt");
	for (int i = 0; i < SIZEY; i++) {
		for (int j = 0; j < SIZEX; j++) {
			fin.get(initialMaze[i][j]);
		}
...
{ //move spot in required direction
	...
	case POWERPILL:
		spot.symbol = '$';
		movementcount = level.numberOfProtectedMoves;
		stats.lives = stats.lives + 1;
		spot.y += dy;
		spot.x += dx;
...
{ //display game title, messages, maze, spot and other items on screen
	...
	showMessage(clDarkGrey, clYellow, 40, 14, "TO FREEZE ZOMBIES PRESS 'F'");
	showMessage(clDarkGrey, clYellow, 40, 15, "TO KILL ZOMBIES PRESS 'X'");
	showMessage(clDarkGrey, clYellow, 40, 16, "TO EAT ALL PILLS PRESS 'E'");
	showMessage(clDarkGrey, clYellow, 40, 17, "TO LOAD GAME PRESS 'L'");
	showMessage(clDarkGrey, clYellow, 40, 18, "TO SAVE GAME PRESS 'S'");
...
void endProgram(Stats stats, string username, bool cheated)
{
	...
	if (stats.lives == 0) showMessage(clDarkGrey, clYellow, 40, 7, "YOU LOST!                  ");
		...
		showMessage(clDarkGrey, clYellow, 40, 7, "YOU WON WITH A SCORE OF "+ toString(stats.score));
		if (stats.score > stats.highscore && !cheated) {
			ofstream fout;
